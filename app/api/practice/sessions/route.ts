...
export async function GET() {
  const { data, error } = await scribeDb
    ...
    .select("id, exercise_id, date, bpm, seconds, note, created_at")
...
// POST { exercise_id, date "yyyy-mm-dd", bpm?, seconds?, note? }
export async function POST(req: NextRequest) {
  if (!validPracticeToken(req.nextUrl.searchParams.get("token"))) return unauthorized();
  const { exercise_id, date, bpm, seconds, note } = await req.json();
...
// PATCH { id, date?, bpm?, seconds?, note? } — null clears bpm/seconds/note.
